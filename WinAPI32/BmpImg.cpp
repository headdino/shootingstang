#include "stdafx.h"
#include "BmpImg.h"
#pragma comment(lib,"msimg32.lib")//알파블렌드 사용하기위해 라이브러리 추가
...
HRESULT BmpImg::init(int width, int height)
{
	...
	//새로 만든 비트맵
	m_imageInfo2->hBit = static_cast<HBITMAP>(CreateCompatibleBitmap(hdc, m_imageInfo->width, m_imageInfo->height));
	//dc에게서 기존걸 받아오고 새로 만든걸 줌 기존정보는 올드 비트맵이라 해놓고 저장
	...
	//알파블렌드 옵션
	m_blenFunc.BlendFlags = 0;
	m_blenFunc.AlphaFormat = 0;// 32비트 비트맵의 경우 AC_SRC_APLHA 라고 초기화 해줘야 한다
	m_blenFunc.BlendOp = AC_SRC_OVER;
...
void BmpImg::render(HDC hdc, int destX, int destY, int destWidth, int destHeight, int sourX, int sourY, int sourWidth, int sourHeight)
{
	...
	{
		//이부분이 현재 늘리거나 축소하면 이미지색상이 깨짐
		//bitblt 에서 늘이기 줄이기 가능한 기능 추가된 함수
...
{
	m_blenFunc.SourceConstantAlpha = alpha;

	//배경색 없앨거냐?
	if (m_isTranse)
	{
		//1. 출력해야 될 dc에 그려져있는 내용을 2번 dc에 옮겨 담는다
		BitBlt(m_imageInfo2->hMemDC, 0, 0, m_imageInfo->width, m_imageInfo->height,
			hdc, 0, 0, SRCCOPY);
		//2. 출력해야될 이미지를 특정색상 제외후 2번 dc 위에 그려준다
		...
		//3. 알파 블렌드 사용하여 출력할 dc에 그린다
		AlphaBlend(hdc, 0, 0, m_imageInfo->width, m_imageInfo->height, 
			m_imageInfo2->hMemDC,0, 0, m_imageInfo->width, m_imageInfo->height, m_blenFunc);
	...
	else
	{
		AlphaBlend(hdc, 0, 0, m_imageInfo->width, m_imageInfo->height, m_imageInfo->hMemDC,
			0, 0, m_imageInfo->width, m_imageInfo->height, m_blenFunc);
	}
}
...
void BmpImg::alphaRender(HDC hdc, BYTE alpha, int destX, int destY, int destWidth, int destHeight, int sourX, int sourY, int sourWidth, int sourHeight)
{
	...
	if (m_isTranse)
	{
		...
		AlphaBlend(hdc, destX, destY, destWidth, destHeight,
			m_imageInfo2->hMemDC, 0, 0, destWidth, destHeight, m_blenFunc);
	}
	//원본 이미지 그대로 출력할거냐?
	else
	{
		AlphaBlend(hdc, destX, destY, destWidth, destHeight, m_imageInfo->hMemDC,
			sourX, sourY, sourWidth, sourHeight, m_blenFunc);
	}
}
...
void BmpImg::frameAlphaRender(HDC hdc, BYTE alpha, int frameX, int frameY, int destX, int destY, int destWidth, int destHeight)
{
	...
	{
		AlphaBlend(hdc, destX, destY, destWidth, destHeight, m_imageInfo->hMemDC,
			sourX, sourY, m_imageInfo->FrameWidth, m_imageInfo->FrameHeight, m_blenFunc);
	}
}
